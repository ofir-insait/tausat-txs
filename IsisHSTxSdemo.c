...
static Boolean HSTxsMSSSendFramesTest(Boolean use_spi)
{
	...
	printf("\n\r start number: [0 - 235]\n\r");

	while(UTIL_DbguGetIntegerMinMax(&selection, 0, 100000) == 0);
	...
	for(vcid = 1; vcid <= 7; vcid++)
	{
		...
		else
		{
			...
			const unsigned NUM_BYTES_TO_WRITE = 3;
			for (frame_count = 0; frame_count < 72; frame_count++) {
				const uint8_t *buf_base = i2c_frame.fields.data + frame_count * NUM_BYTES_TO_WRITE;
				const uint8_t val = (uint8_t)frame_count + (uint8_t)selection;
				memset(buf_base, val, NUM_BYTES_TO_WRITE);
			}
...
//			memset(i2c_frame.fields.data, 0x0, sizeof(i2c_frame.fields.data));
//			for(frame_count = selection; frame_count < selection + 20; frame_count++) {
//				const int buffer_base = frame_count - selection;
//				memset(i2c_frame.fields.data + buffer_base * 5, (uint8_t)frame_count, 5);
//			}
//			for(;;)
//			{
//				isis_hstxs__write_and_send_frame(0, &i2c_frame, frame_payload_size, &status_out);
...
//				for(byte_count = 0; byte_count < frame_payload_size; byte_count++)
//				{
//					i2c_frame.fields.data[byte_count] = 0x4;
//				}
//				isis_hstxs__write_and_send_frame(0, &i2c_frame, frame_payload_size &status_out);
//			}
//			}
...
typedef struct inklajn_spl_TM
{
	...
	int time;//Unix time
	char data[227];//the data in the packet
}TM_spl;
Boolean _watchdog__tm( void );
#define CONST_FLAG 0x01010202
#define TYPE (1)
#define SUB_TYPE (2)
#define LENGHT 30>>8
static void sendSplPacketI2CTxs()
{
	isis_eps__demo__init();
	const int frame_payload_size = 217;
	...
	i2c_frame.fields.data = malloc(frame_payload_size);
	_watchdog__tm();
	ccsds_header.fields.virtual_channel=1;
	i2c_frame.fields.header = ccsds_header;

//	TM_spl packet={TYPE,SUB_TYPE,LENGHT,CONST_FLAG};
	TM_spl packet={TYPE,SUB_TYPE,30,CONST_FLAG};
	packet.time = CONST_FLAG;
	memset(packet.data, 0x0, sizeof(packet.data));
	unsigned int freq_2200000 = 2200000;

#if 0
	memcpy(i2c_frame.fields.data,&packet, frame_payload_size);
	for(j =0; j<30;j++)
	{
		i2c_frame.fields.data[j]=1;
	}
#else
	const int magic1 = 0xdeadbeef;
	const int magic2 = 0xdabad00d;
	for(j = 0; j < 32; j+= 4)
	{
		if (j % 8 == 0)
			memcpy(packet.data + j, &magic1, sizeof(magic1));
		else
			memcpy(packet.data + j, &magic2, sizeof(magic2));
	}
	packet.length = 32;
	memcpy(i2c_frame.fields.data,&packet, frame_payload_size);
...
//	for(i = 0, j =30; j < 150; j++, i++)
//	{
//		if (i < 30)
//			i2c_frame.fields.data[j]=i;
//		else
//			i2c_frame.fields.data[j]=0;
//	}

	isis_hstxs__set_sup_mode(0,isis_hstxs__sup_mode__standby);
	vTaskDelay(1000);
	isis_hstxs__set_tx_frequency(0, freq_2200000);
	vTaskDelay(1000);
	isis_hstxs__set_tx_mode(0, isis_hstxs__mode__on);
	vTaskDelay(100);
	while(!DBGU_IsRxReady())
	{
		isis_hstxs__write_and_send_frame(0, &i2c_frame, frame_payload_size, &status_out);
		if(xTaskGetTickCount() % 100000 == 0)
		{
			_watchdog__tm();
		}
	}
	isis_hstxs__set_tx_mode(0, isis_hstxs__mode__standby);
	isis_hstxs__set_sup_mode(0,isis_hstxs__sup_mode__sup_only);
